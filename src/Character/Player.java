...
public class Player extends Unit
{
	...
	/** The player's level schedule, how much xp is needed to level up from each level */
	...
	throws SlickException
	{
		...
		this.pickupItem( new InventoryWeapon( "Short Sword", new Image("assets/items/sword.png"), Bonus.EMPTY_BONUS, 5, 10) );

		ArrayList<Bonus> potionBonus = new ArrayList<Bonus>(1);
		potionBonus.add(new Bonus(Bonus.Type.HP, 25));
		this.pickupItem( new InventoryConsumable( "Health Potion", new Image("assets/items/potion.png"), potionBonus) );
	 ...
	 * @param game The game object, which acts as a facade. 
	 */
	public void update(int dirX, int dirY, int delta, GameplayState gps)
	throws SlickException
	{
		// Handle player death
		if(this.getHealth() <= 0)
	 ...
	 * 
	 * @param gps The gameplay state to check in
	 ...
	 * 
	 * @param item The WorldItem to be converted and added.
	 * @param world The world from which to remove the WorldItem.
	 */
	public void pickupItem(WorldItem item, GameplayState gps)
	{
		...
		else
		{
			...
			if(item.getType() == Item.Type.CONSUMABLE)
			{
				...
				this.getInventory().addItem( new InventoryConsumable(cons.getName(), cons.getAvatar(), cons.getBonuses()) );
			}
			// If it's an accessory
			else if(item.getType() == Item.Type.ACCESSORY)
			{
				this.getInventory().addItem( new InventoryAccessory(item.getName(), item.getAvatar(), item.getBonuses()) );
			}
			// If it's a weapon
			else if(item.getType() == Item.Type.WEAPON)
			{
				WorldWeapon weapon = (WorldWeapon)(item);
				this.getInventory().addItem( new InventoryWeapon(weapon.getName(), weapon.getAvatar(), weapon.getBonuses(),
			...
			else if(item.getType() == Item.Type.QUEST)
			{
				...
				this.getInventory().addItem( new InventoryQuest(quest.getName(), quest.getAvatar(), quest.getBonuses()) );
			}
			// If it's a piece of armour
			else if(item.getType() == Item.Type.ARMOUR ||
					item.getType() == Item.Type.SHIELD ||
					item.getType() == Item.Type.HELMET ||
					item.getType() == Item.Type.GLOVES ||
					item.getType() == Item.Type.BOOTS )
			{
				WorldArmour armour = (WorldArmour)(item);
				this.getInventory().addItem( new InventoryArmour(armour.getName(), 
						armour.getType(), armour.getAvatar(), armour.getBonuses(), armour.getArmourRating()) );
			}
			else
			{
	 ...
	 * 
	 * @param world The world to pickup the item from
	 */
	 ...
	 * 
	 * @param gps The game world, which holds the list of monsters.
	...
	Monster scanForMonster(GameplayState gps)
	{
		...
		{
			// Early failure check - if diff in either axis is too great, move on 
	 ...
	 * 
	 * @param gps The game world, which holds the list of world items.
     ...
     * 
     * @param gui The GUI to select from
     */
    ...
    /** Drop an item from the inventory panel. Essentially reverses the pickupItem() method.
     * 
     * @param gps Facade for GUI, and world to drop item into
     * @throws SlickException 
     */
    public void dropInvItem(GameplayState gps) throws SlickException
    {
		...
		if(item != null)
		{
			...
			if(item.getType() == Item.Type.CONSUMABLE)
			{
				...
				gps.spawnWorldItem( new WorldConsumable(this.getX(), this.getY(), cons.getName(), cons.getAvatar(), cons.getBonuses()) );
			}
			// If it's an accessory
			else if(item.getType() == Item.Type.ACCESSORY)
			{
				gps.spawnWorldItem( new WorldAccessory(this.getX(), this.getY(), item.getName(), item.getAvatar(), item.getBonuses()) );
			}
			// If it's a weapon
			else if(item.getType() == Item.Type.WEAPON)
			{
				InventoryWeapon weapon = (InventoryWeapon)(item);
				gps.spawnWorldItem( new WorldWeapon(this.getX(), this.getY(), weapon.getName(), weapon.getAvatar(),
						weapon.getBonuses(), weapon.getMinDamage(), weapon.getMaxDamage()) );
			}
			// If it's a quest item
			else if(item.getType() == Item.Type.QUEST)
			{
				InventoryQuest quest = (InventoryQuest)(item);
				gps.spawnWorldItem( new WorldQuest(this.getX(), this.getY(), quest.getName(), quest.getAvatar(), quest.getBonuses()) );
			}
			// If it's a piece of armour
			else if(item.getType() == Item.Type.ARMOUR ||
					item.getType() == Item.Type.SHIELD ||
     ...
     * 
     * @param gps The GPS to drop into
    ...
    public void dropGold(GameplayState gps, int amount)
    {
    	...
    	this.gold -= amount;
		// Debug print
		System.out.println(getName()+" dropped "+amount+ "gold @ ["+this.getX()+", "+this.getY()+"]");
    }
	...
	public double getMinDamage()
	{
		...
		InventoryWeapon weapon = (InventoryWeapon)(this.getEquipment().getItems().get(Item.Type.WEAPON));
		double damage;
		// If holding a weapon
		if(weapon != null)
		{
