...
import Inventory.InventoryWeapon;
...
public class GUI
{
	 ...
	 * 
	 * @param delta The time (ms) since the last frame
	 */
	 ...
	 * @param p The player whose status should be displayed
	 */
	...
	/** Renders the player's status panel.
	 * @param g The current Slick graphics context
	 * @param p The player, whose inventory should be displayed
	 */
	public void renderInventoryPanel(Graphics g)
	{
		...
		if(item != null)
		{
			...
			// Write the item name
			float textX = initInvX + (INVPANELWIDTH - 20)/2 - g.getFont().getWidth(item.getName())/2;
			float textY = 361;
	...
	private String toTitleCase(String in)
	{
		...
		for (int j = 0; j < in.length(); j++)
		{
			...
			if (j == 0)	title += next.toUpperCase();
			else		title += next.toLowerCase();
		}
		return title;
	}
