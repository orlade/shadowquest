...
public class GameplayStateLevelDungeon extends GameplayState
{
	...
	public void initMonsters() throws SlickException
	{
		...
		System.out.println("mapsize="+mapSize+", ["+this.getMap().getMapWidth()+", "+this.getMap().getMapHeight()+"]");
		...
		for(int i=0; i < numMonsters; i++)
		{
			...
			Monster monster;
			double mx = Math.random() * (getMap().getMapWidth()-2*getMap().getTileWidth()) + getMap().getTileWidth();
			double my = Math.random() * (getMap().getMapHeight()-2*getMap().getTileHeight()) + getMap().getTileHeight();
			...
			case 0:
				monster = new MonsterZombie(0, 0, (int)(getPlayer().getLevel() + Math.random()*7-4) );
				break;
			// Bandit (mlvl = [plvl-2, plvl+4])
			case 1:
				monster = new MonsterBandit(0, 0, (int)(getPlayer().getLevel() + Math.random()*7-2) );
				break;
			// Skeleton (mlvl = [plvl+1, plvl+7])
			case 2:
				monster = new MonsterSkeleton(0, 0, (int)(getPlayer().getLevel() + Math.random()*7+1) );
				break;
			...
			// Place the monster in a non-blocked tile, outside of player detection range
			...
			{
				mx = Math.random() * (getMap().getMapWidth()-2*getMap().getTileWidth()) + getMap().getTileWidth();
				my = Math.random() * (getMap().getMapHeight()-2*getMap().getTileHeight()) + getMap().getTileHeight();
				//System.out.println("MAP = ("+getMap().getMapWidthTiles()+", "+getMap().getMapHeightTiles()+"), MONSTER = ("+mx/72+", "+my/72+")");
			}
			monster.setX(mx);
			monster.setY(my);
