...
import Character.Monster.MonsterBandit;
import Character.Monster.MonsterNecromancer;
import Character.Monster.MonsterSkeleton;
import Character.Monster.MonsterZombie;
import Character.Villager.Villager;
import Character.Villager.VillagerPeasant;
import Character.Villager.VillagerPrince;
import Character.Villager.VillagerShaman;
import Character.Villager.VillagerShopkeeper;
import Inventory.InventoryConsumable;
import World.ExitZone;
import World.WorldAccessory;
import World.WorldConsumable;
import World.WorldItem;
import World.WorldMap;
import World.WorldQuest;
import World.WorldWeapon;
...
public class GameplayStateLevelMain extends GameplayState
{
			...
			throws SlickException
	{
        ...
        this.getMap().setExit(new ExitZone(
        		13.0 * getMap().getTileWidth(), 6.0 * getMap().getTileHeight(),
	...
	public void update(GameContainer gc, StateBasedGame sbg, int delta)
			throws SlickException
	{
		// Handle buying from shopkeeper - should be implemented somewhere else probably
			...
			for(Villager v : villagers)
			{
				if(v instanceof VillagerShopkeeper && v.isInteracting())
				{
					if(getPlayer().getGold() >= VillagerShopkeeper.potionPrice)
					{
						...
						getPlayer().pickupItem( new InventoryConsumable(
								"Health Potion", new Image("assets/items/potion.png"), potionBonus) );
	...
	public void initItems() throws SlickException
	{
		...
		ArrayList<Bonus> debugBonus = new ArrayList<Bonus>(1);
		debugBonus.add(new Bonus(Bonus.Type.HPREGEN, 1));
		debugBonus.add(new Bonus(Bonus.Type.SPEED, 0.25));
		debugBonus.add(new Bonus(Bonus.Type.PCDAMAGE, 0.8));
		// Create the item
		items.add( new WorldArmour(this.getPlayer().getX() + 100, this.getPlayer().getY()+100,
				"Armour of Velocity",
				Item.Type.ARMOUR,
				new Image("assets/items/elven_shield.png"),
				debugBonus,
				50 ) );
		 */
		...
		items.add( new WorldAccessory(this.getPlayer().getX(), this.getPlayer().getY()+100,
				"Amulet of Epic Debug",
				new Image("assets/items/amulet.png"),
				epicDebugBonus ) );
		...
		// Create the item
		items.add( new WorldAccessory(972 , 2916,
				"Amulet of Vitality",
				new Image("assets/items/amulet.png"),
				vitalityBonus ) );
		...
		// Create the item
		items.add( new WorldWeapon(1980, 1476,
				"Sword of Strength",
				new Image("assets/items/sword.png"),
				strengthBonus,
				15, 30) );
		...
		// Create the item
		items.add( new WorldConsumable(2052, 900 ,
				"Tome of Agility",
				new Image("assets/items/book.png"),
				agilityBonus ) );

		// Create the item (no bonus)
		items.add( new WorldQuest(2052, 396 ,
				"Elixir of Life",
				new Image("assets/items/elixir.png"),
				Bonus.EMPTY_BONUS ) );
	}
