...
public abstract class Map
{
     ...
     * @param cameraX The camera's X coordinate (from World)
     * @param cameraY The camera's Y coordinate (from World)
     */
     ...
     * 
     * @param g The Slick graphics object, used for drawing.
     * @param tileOffsetX The tilemap's X offset
     * @param tileOffsetY The tilemap's Y offset
     * @param tileIndexX The tilemap's horizontal starting tile index
     * @param tileIndexY The tilemap's vertical starting tile index
     * @param visX The number of tiles being rendered horizontally
     * @param visY The number of tiles being rendered vertically
     */
	private void renderBlockMask(Graphics g, int tileOffsetX, int tileOffsetY, int tileIndexX, int tileIndexY, int visX, int visY)
	{
        ...
        for (int xp=0; xp<getMap().getWidth(); xp++)
        {
        	...
        	{ 
        		if ((xp >= tileIndexX) && (yp >= tileIndexY) && xp < tileIndexX + visX && yp < tileIndexY + visY)
        		{ 
        			if (isTileBlocked(xp, yp))
        			{
        				g.fillRect(
        						tileOffsetX + (xp * tileWidth) - (tileIndexX * tileWidth),
     ...
     * 
     * @param x The x coordinate of the centre of the box being tested.
     * @param y The y coordinate of the centre of the box being tested.
     * @param width The width of the box being tested.
     * @param height The height of the box being tested.
     * @param game The parent Game object used as a facade.
     */
	public boolean isBoxBlocked(double x, double y, int width, int height)
	{
		...
		// Test is these tiles have their "block" property set to "1", return result
	...
	{
		if( entrance != null && 
			x > entrance.getX() && x < entrance.getX() + entrance.getWidth() &&
			y > entrance.getY() && y < entrance.getY() + entrance.getHeight() )
		{
			return entrance;
		}
		else if( exit != null &&
				x > exit.getX() && x < exit.getX() + exit.getWidth() &&
