...
//		labelFont = FontLoader.loadFont("assets/DejaVuSans-Bold.ttf", 12);
	...
	{ // Update the player's movement direction based on keyboard presses.
		...
		if (isDevMode())
		{
			...
			/*
			 * // Jump to/from dungoen if(getInput().isKeyPressed(Input.KEY_J))
			 * { switch(sbg.getCurrentStateID()) { case RPG.GPSMAIN:
			 * sbg.enterState(RPG.GPSDUNGEON, null, new
			 * FadeInTransition(Color.black)); break; case RPG.GPSDUNGEON:
			 * sbg.enterState(RPG.GPSMAIN, null, new
			 * FadeInTransition(Color.black)); break; } } // Reset the current
			 * level if(getInput().isKeyPressed(Input.KEY_R)) { // Clear the
			 * existing entities getMonsters().clear(); getCorpses().clear();
			 * getVillagers().clear(); getWorldItems().clear();
			 * 
		...
		if (getInput().isKeyPressed(Input.KEY_SPACE) && getPlayer().getPickupCandidate() != null)
		{
		...
		// If the inventory panel is open, squash the camera so the player is
		// still centred
		if (isRenderInvPanel())
		{
...
//		gc.getGraphics().setFont(this.getLabelFont());
	...
	/**
	 * Switches the GPS to a new level, transferring the player object to the
	 * map 'entrance'
	 * 
	 * @param levelId
	 *            The ID of the new level
	 * @param player
	 *            The player object for persistence
	 */
	public void changeLevel(int levelId)
	{
		// Enter the new state
		this.getParentGame().enterState(levelId, new FadeOutTransition(Color.black),
	...
	public void changeLevel(ExitZone z)
	{
		...
		// Enter the new state
		this.getParentGame().enterState(z.getTargetId(), new FadeOutTransition(Color.black),
				new RefocusFadeInTransition(Color.black, z));
	}

	/**
	 * Updates the target state to set up the world correctly given the player's
	 * expected position
	 * 
	 * @param fs
	 *            The first state, i.e. the state being switch INTO
	 * @param ss
	 *            The second state, i.e. the state being switch OUT FROM
	 * @param z
	 *            The exit zone being jumped to, if any
	 */
	public void changeLevelRefocus(GameplayState fs, GameplayState ss, ExitZone z)
	{
		...
		// If going UP, set the player's position to that of the last exit he
		// used
		...
		else
		{
			...
			System.out.println("FADEIN: Player set to (" + fs.getPlayer().getX() + ", "
					+ fs.getPlayer().getY() + ")");
			System.out.println("FADEIN: NOT ("
					+ fs.getMap().getEntrance().getX()
					+ ", "
					+ fs.getMap().getEntrance().getX()
					+ ") ["
			...
			// If the entrance tile is blocked (probably because it's undefined,
			// defaulting to (0,0)), find a new one
					...
					fs.getPlayer().getPhysWidth(), fs.getPlayer().getPhysHeight()))
			{
				...
				{
					fs.getPlayer().setX(
							(int) (Math.random() * (fs.getMap().getMapWidthTiles() - 2) + 1)
									* fs.getMap().getTileWidth() - fs.getMap().getTileWidth() / 2);
					fs.getPlayer()
							.setY((int) (Math.random() * (fs.getMap().getMapHeightTiles() - 2) + 1)
	 ...
	 * 
	 * @param dir
	 *            A char representing the direction of movement (u, d, l, r)
	 */
