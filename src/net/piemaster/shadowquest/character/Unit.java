...
import net.piemaster.shadowquest.inventory.Inventory;
...
public abstract class Unit implements WorldObject
{
	...
	public void attack(Unit target)
	{
		if (cooldownTimer <= 0)
		{
			...
			double hitChance = 100 * 2 * (double) this.getLevel()
					/ (this.getLevel() + target.getLevel());
			if ((Math.random() * 100) > 100 - hitChance
					- this.getBonuses().get(Bonus.Type.ACCURACY))
			{
				...
				// Debug print
				System.out.println("POW! [" + this.getName() + " attacked " + target.getName()
						+ " for " + damage + " dmg]");
	 ...
	 * 
	 * @param newX
	 *            The x coordinate of the proposed destination.
	 * @param newY
	 *            The x coordinate of the proposed destination.
	 * @param gps
	 *            The GameplayState that holds the map for map checks.
	 */
	...
	public void renderSecondary(Graphics g)
	{
		...
		/*
		 * //DEBUG: Render the unit's physical bounding box g.setColor(new
		 * Color(0,1,0,0.5f)); // Map is drawn before graphics are translated
		 * g.fillRect((float)(getX()-getPhysWidth()/2),
		 * (float)(getY()-getPhysHeight()/2), (float)getPhysWidth(),
		 * (float)getPhysHeight());
		 */
	}
	...
	public void renderHealthBox(Graphics g)
	{
		...
		g.setColor(BAR);
		g.fillRect(barX, barY, (healthBarWidth * ((float) getHealth() / getMaxHealth())),
				healthBarHeight);
	 ...
	 * 
	 * @param gps
	 *            The GameplayState to remove the unit from
	 */
	 ...
	 * 
	 * @param gps
	 *            The GameplayState to add the corpse to
	 */
	...
	/**
	 * Gets the total armour rating of the unit, the sum of all their equipped
	 * armour
	 * 
	 ...
	 * 
	 * @param amount
	 *            Amount of health to give to the unit
	 */
	 ...
	 * 
	 * @param amount
	 *            Amount of energy to give to the unit
	 */
