...
import net.piemaster.shadowquest.world.RandomFactory;
...
public abstract class Monster extends Unit
{
	 ...
	 */
	public Monster(double x, double y, int level) throws SlickException
	{
	 ...
	 * @param gps
	 *            The GameplayState, which acts as a facade.
	 */
	@Override
	 ...
	 * 
	 * @param gps
	 *            The GameplayState, which holds the lists of units.
	...
	public void die(GameplayState gps)
	{
		...
		gps.getMonsters().remove(this);// (gps.getMonsters().indexOf(this),
										// null);
	 ...
	 * 
	 * @param gps
	 *            The GameplayState to create the item in.
	 */
	public void dropItem(GameplayState gps)
	{
		...
		if (rand < 0.5)
		{
			double dropX = this.getX() + Math.random() * this.getPhysWidth()
					* (1 - Math.round(Math.random()) * 2);
			double dropY = this.getY() + Math.random() * this.getPhysHeight()
					* (1 - Math.round(Math.random()) * 2);
			...
			if (gps.getMap().isTileBlocked(dropX, dropY))
			{
				System.out.println("Drop location [" + dropX + ", " + dropY
						+ "] blocked! Dropping at current position");
				dropX = this.getX();
				dropY = this.getY();
			}
			int amount = (int) (10 + (Math.random() * 10 * this.getLevel())
					* gps.getPlayer().getGoldDrop());
			try
			{
		...
		if (rand < gps.getPlayer().getItemDrop())
		{
			...
			try
			{
				...
				case 5:
				case 6:
					gps.spawnWorldItem(RandomFactory.createArmourGeneral(dropX, dropY,
							this.getLevel(), Item.chooseRandomArmourType()));
					break;
				// Accessory
				case 7:
